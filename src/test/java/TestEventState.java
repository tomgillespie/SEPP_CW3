...
import static org.junit.jupiter.api.Assertions.*;
    ...
    public class TestEventState {
        ...
        //The tests we implemented are as follows:
        //1. test all events returns a null list when initialised
        //2. test create non ticketed event with valid inputs
        //3. test create ticketed event with valid inputs
        //4. test get next event number starts at 1
        //5. test get next event number increments
        //6. test get next performance number starts at 1
        //7. test create an event performance with valid inputs
        //8. test get by event number with correct event number
        //9. test get by event number with  event number on limit
        //10. test get by event number with invalid input
        ...
        public void testCreateNonTicketedEvent () {
            ...
            eventState.createNonTicketedEvent(organiser, "The games", EventType.Sports);
            assertEquals(1, eventState.getAllEvents().size());
        }

        @Test
        @DisplayName("Test the ticketed event booking works")
        public void testcreateTicketedEventBooking () {
            EntertainmentProvider organiser = new EntertainmentProvider(
                    "TestOrganiser",
                    "home",
                    "@email.com",
                    ...
                    List.of("person1", "person2"),
                    List.of("emailing@email.yahoo.com"));
            eventState.createTicketedEvent(organiser, "The games", EventType.Sports, 11.99, 100);
            assertEquals(1, eventState.getAllEvents().size());
        }
        ...
        public void testGetNextEventNumberItterates () {
            ...
            eventState.createTicketedEvent(organiser, "The games 2.0", EventType.Sports, 20, 2000);
        ...
        public void testCreateEventPerformance () {
            ...
            eventState.createEventPerformance(eventState.getAllEvents().get(0),
                    ...
                    100,
                    200);
            assertEquals(1, eventState.getAllEvents().get(0).getPerformances().size());
        }
        ...
        public void testGetEventByNumber () {
            ...
            eventState.createNonTicketedEvent(organiser, "The games", EventType.Sports);
            eventState.createTicketedEvent(organiser, "Formula 1", EventType.Sports, 11.99, 100);
            eventState.createNonTicketedEvent(organiser, "Royal Variety Show", EventType.Dance);
